...
const isMember = (uid, callback) => {
  const next = (err, data) => {
    const isInAnyGroup = data.indexOf(true) !== -1;
    callback(err, isInAnyGroup);
  };
  if (Array.isArray(uid)) {
    groups.isMembersOfGroupsList(uid, ['casuals', 'raiders', 'officers'], next);
  } else {
    groups.isMemberOfGroupsList(uid, ['casuals', 'raiders', 'officers'], next);
  }
};
    ...
    (next) => async.series({
      apps: (next) => {
        ...
        async.waterfall([
          ...
          (data, next) => {
            apps = data;
            uids = Object.keys(_.groupBy(apps, app => app.uid));
...
Application.getApplication = (uid, appId, callback) => {
  ...
  async.waterfall([
    ...
    (next) => {
      db.isSortedSetMember("uid:" + uid + ":mnl:appid", uid, next);
    },
    (isAuthor, next) => {
...
const createReply = (uid, appId, message, timestamp, callback) => {
  let reply;
  async.waterfall([
    (next) => {
...
Application.createReply = (uid, data, callback) => {
  ...
  async.waterfall([
    ...
    (next) => {
      db.getObjectField("mnl:application:" + data.appId, "status", (err, status) => {
        next(err, parseInt(status, 10));
      });
    },
    (status, next) => {
    ...
    (next) => {
      createReply(uid, data.appId, message, timestamp, callback);
    },
  ], (err, result) => {
    if (err) {
      callback(err);
