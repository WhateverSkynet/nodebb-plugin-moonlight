...
Controllers.renderAdminPage = function (req, res, next) {
  ...
  */
  Recruitment.getRecruitment((err, classes) => {
    if (err) {
      console.log(err)
    }
    const data = {
      action: {
        type: '@mnl/ajaxify/RECRUITMENT',
        classes: classes
      }
    }
    res.render('admin/plugins/moonlight', data)
  })
}

Controllers.renderApplicationsPage = function (req, res, next) {
  /*
   Make sure the route matches your path to template exactly.
   ...
   and you would render it like so:
    res.render('some/complex/route')
  */
  Application.getApplications(req.uid, (err, payload) => {
    if (err) {
      console.log(err)
    }
    const action = {
      type: '@mnl/ajaxify/APPLICATION_LIST',
      payload
    }
...
Controllers.renderApplicationPage = function (req, res, next) {
  /*
   Make sure the route matches your path to template exactly.
   ...
   and you would render it like so:
    res.render('some/complex/route')
  */
  Application.getApplication(req.uid, req.params.id, (err, payload) => {
    const action = {
      type: '@mnl/ajaxify/APPLICATION',
      payload
    }
    if (err) {
      if (err.message === 'Unauthorized.') {
        return res.render('403', { title: '[[global:403.title]]' })
      } else {
        return res.render('404', { title: '[[global:404.title]]' })
      }
    }
    res.render('application', payload.application ? { action } : {})
  })
}

Controllers.renderRosterPage = function (req, res, next) {
  /*
   Make sure the route matches your path to template exactly.
  ...
  */
  ...
  Roster.getRoster(data => {
    res.render('roster', {
      action: {
        type: '@mnl/ajaxify/ROSTER',
        characters: data
      }
    })
  })
...
Controllers.renderLandingPage = function (req, res, next) {
  /*
   Make sure the route matches your path to template exactly.
  ...
  */
  async.parallel([
    ...
    (next) => {
      Blog.getPublishedPosts(req.uid, undefined, (err, posts) => {
        const action = {
          type: '@mnl/ajaxify/BLOG',
          payload: {
            posts
          }
        }
...
Controllers.renderApplyPage = function (req, res, next) {
  Application.getApplicationTemplate(req.uid, (err, template) => {
    if (err) {
      if (err.message === 'Is Member.') {
        // mmbers can't create applications.
        return helpers.redirect(res, '/applications')
      }
    }
    const action = {
      type: '@mnl/ajaxify/NEW_APPLICATION',
      payload: {
        template
      }
    }
...
Controllers.renderWoWData = function (req, res, next) {
  /*
   Make sure the route matches your path to template exactly.
