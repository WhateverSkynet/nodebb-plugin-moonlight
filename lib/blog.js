...
const createObject = (id, uid, {title, imageText, imageUrl, content, date, deleted}) => {
...
const createPost = (uid, data, next) => {
  const {title, imageText, content, date} = data;
  const timestamp = Date.now();
...
const getAllPosts = (uid, data, next) => {
  async.waterfall([
    ...
    (data, next) => {
      ...
      const apps = data.map(praseObject)
        .filter(post => post.deleted === 0);;
      next(null, apps);
    },
  ], (err, result) => {
